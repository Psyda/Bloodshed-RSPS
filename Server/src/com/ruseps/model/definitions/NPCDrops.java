...
import com.ruseps.model.definitions.NPCDrops.WellChance;
...
public class NPCDrops {
	...
	public static class NpcDropItem {
		...
		public DropChance getChance() {
			switch (chance) {
			case 1:
				return DropChance.ALMOST_ALWAYS; // <-> 1/7
			case 2:
				return DropChance.VERY_COMMON; // <-> 1/7
			case 3:
				return DropChance.COMMON; // <-> 1/15
			case 4:
				return DropChance.UNCOMMON; // <-> 1/15
			case 5:
				return DropChance.NOTTHATRARE; // <-> 1/50
			case 6:
				return DropChance.RARE; // <-> 1/100
			case 7:
				return DropChance.LEGENDARY;// <-> 1/200
			case 8:
		...
		public WellChance getWellChance() {
			switch (chance) {
			case 1:
				return WellChance.COMMON; // <-> 1/7
			case 2:
				return WellChance.COMMON; // <-> 1/7
			case 3:
				return WellChance.UNCOMMON; // <-> 1/15
			case 4:
				return WellChance.UNCOMMON; // <-> 1/15
			case 5:
				return WellChance.RARE; // <-> 1/50
			case 6:
				return WellChance.LEGENDARY; // <-> 1/100
			case 7:
				return WellChance.LEGENDARY_1;// <-> 1/200
			case 8:
				return WellChance.LEGENDARY_2;// <-> 1/333
			case 9:
				return WellChance.LEGENDARY_3;// <-> 1/750
			case 10:
				return WellChance.LEGENDARY_4;// <-> 1/1000
			default:
				return WellChance.ALWAYS; // always
			}
		}
	...
	public enum WellChance {
		ALWAYS(0), COMMON(5), UNCOMMON(25),  RARE(100), LEGENDARY(150), LEGENDARY_1(200), LEGENDARY_2(250), LEGENDARY_3(500), LEGENDARY_4(1000);
	...
	public static boolean shouldDrop(boolean[] b, DropChance chance,//DROP RATE BOOSTS FOR EACH RANK
	...
	public static void drop(Player player, Item item, NPC npc, Position pos,
		...
		if(item == null || item.getDefinition() == null || item.getDefinition().getName().equals("null") || (item.getAmount() > 15000 && !item.getDefinition().isStackable() && !item.getDefinition().isNoted())) {
			return;
		}
		...
		shouldOrb = true;
		if (itemId == LootCoin.GOLD_COIN
				|| itemId == LootCoin.GREEN_COIN
				|| itemId == LootCoin.CRIM_COIN
				|| itemId == LootCoin.BLUE_COIN) {
			shouldOrb = false;
			if (player.getEquipment().contains(2572)
					&& LootCoin.handleCharmDrop(player, itemId, amount)) {
				return;
		...
		if (itemId == 995 && amount > 49000000) {
			...
			+ " has just received @red@" +Misc.insertCommasToNumber(String.valueOf(amount))+"x Coins as a rare drop!";
		...
		if (ItemDropAnnouncer.announce(itemId)) {
			...
			if (player.getRights() == PlayerRights.DEVELOPER || player.getRights() == PlayerRights.RUBY_MEMBER || player.getRights() == PlayerRights.DIAMOND_MEMBER || player.getRights() == PlayerRights.ADMINISTRATOR) {
				if(npc.getId() == 3847 || npc.getId() == 2042 || npc.getId() == 2043 || npc.getId() == 2044) {
	...
	public static void casketDrop(Player player, int combat, Position pos) {
		...
		if (RandomUtility.getRandom(combat <= 50 ? 1300 : 1000) < chance) {
			...
			player.getPacketSender().sendMessage("@or2@You have recieved a casket!");
		}
	}
	private static final int[] CLUESBOY = new int[] { 2677, 2678, 2679, 2680, 2681, 2682, 2683, 2684, 2685, 2686, 2687, 2688, 2689, 2690};
	...
	public static class ItemDropAnnouncer {
		...
		private static final int[] TO_ANNOUNCE = new int[] { 14484, 4212,
			11702, 11704, 11706, 11708, 11704, 12646, 11724, 19780, 10548, 15241, 15243,  11730, 11726, 11728, 11718,
			11720, 11722, 11730, 11967, 13247, 12655, 11969, 11970, 13058, 11971, 11972, 11973, 11974, 11975, 11976, 11716, 14876, 11286, 13427, 
			 2513, 15259, 13902, 13890, 13884, 13861, 13858,
			13864, 13905, 13887, 13893, 17927, 13235,  7986, 7981, 7980, 12708, 13239, 13045, 13047, 13899, 13051, 2572, 11614, 12601, 4565, 1038, 1040, 1042, 1044, 1046, 1048, 962, 4566, 113873, 13879, 13876, 13870,
			6571, 14008, 14009, 14010, 14011, 20555, 11283, 13263, 11716, 6500, 14012, 2572, 12926, 1037, 1053, 1055, 1057, 1050, 11862, 13051, 11860, 11858, 5607, 15501, 13058, 12601, 12603, 12605,  14013, 14014, 14015,
			14016, 13750, 13748, 17291, 13746, 15490, 15488, 13752, 13740, 18349, 18351, 6570, 12284, 18353, 18355, 18357, 18359, 13263, 11283, 11694, 14484, 11335, 15486, 13870, 13873,
			13876, 13884, 13890, 13896,  14479,11554, 4151, 4084, 4705, 4706, 2572, 11848, 11700,11694, 11730, 11698, 11696, 11850, 11852, 11854, 11856, 11846, 13902, 13858, 13861, 13864, 13867,
			11995, 11996, 11997, 11978, 12001, 12002, 13742, 12003, 12004, 19335, 12005,
			12006, 11990, 11991, 11992, 11993, 11994, 13744, 13738, 11989, 11988, 11987, 12282,
			11986, 11985, 11984, 11983, 11982, 11981, 11979, 13659,
			20000, 12927, 20001,11613, 11287,  20002, 16425};
