...
public class Locations {
	...
	// so the inferno source doesnt have a client , and the "server" folder is your
	// hosting source right
	// so yeah inferno source is the leaked one i got i have cache but not client
	// for it and source is
	// one we are running the inferno source is what your running? we are running
	// server we need inferno of
	// inferno source kk

	public enum Location {
		INFERNO(new int[] { 2244, 2300 }, new int[] { 5313, 5370 }, true, false, true, false, false, true) {
			@Override
			...
			public void logout(Player player) {
				if (player.getRegionInstance() != null
						&& player.getRegionInstance().equals(RegionInstanceType.INFERNO)) {
					player.getRegionInstance().destruct();
					World.getNpcs().forEach(
							n -> n.removeInstancedNpcs(Locations.Location.INFERNO, player.getPosition().getZ()));
			...
			public void process(Player player) {
				if (player.getPosition().getY() >= 5359) {
					player.moveTo(new Position(player.getPosition().getX(), 5358, player.getPosition().getZ()));
				}
		...
		BRIDGE(new int[] { 3361, 3373 }, new int[] { 3935, 3936 }, true, true, true, false, true, true) {
		},
		GAMBLING(new int[] { 2400, 2470 }, new int[] { 3070, 3140 }, false, false, true, true, true, true) {
			/*
			 * @Override public void enter(Player player) { if
			 * (player.getPlayerInteractingOption() != PlayerInteractingOption.GAMBLE)
		...
		SLASH_BASH(new int[] { 2457, 2487 }, new int[] { 9430, 9454 }, true, true, true, true, true, true) {
		...
		RZ(new int[] { 2304, 2335 }, new int[] { 4607, 3567 }, false, false, false, false, false, false) {
		},
		...
		WILDERNESS(new int[] { 2941, 3392, 2986, 3012, 3653, 3710, 3650, 3653 },
			...
			public void enter(Player player) {
				...
				if (Misc.getMinutesPlayed(player) <= 30) {
					...
					TeleportHandler.teleportPlayer(player, new Position(3093, 3503),
		...
		// RZ(new int[]{2304, 2335}, new int[]{4607, 3567}, false, false, false, false,
		// false, false) {
		public static boolean inRZ(Character gc) {
			if (gc.getLocation() == RZ) {
				int x = gc.getPosition().getX(), y = gc.getPosition().getY();
				if (x >= 2304 && x <= 2335 && y >= 4607 && y <= 3567)
					return true;
			}
