...
public class NPCDeathTask extends Task {
	...
	private Set<Integer> BOSSES = new HashSet<>(Arrays.asList(
			1999,2044,2043,2042,2882,2881,2883, 7134, 3847, 3340, 6766, 5666, 7286, 4540, 6222, 6260, 6247, 6203, 8349,50,2001,11558,4540, 1158, 8133, 3200, 13447, 8549,3851,1382,8133,13447,2000,2009,2006
	...
	public void execute() {
		try {
			...
			case 2043:
			case 2044:
				//TeleportHandler.teleportPlayer(killer, new Position(3087 + Misc.getRandom(10), 3502 + Misc.getRandom(2)), killer.getSpellbook().getTeleportType());
				killer.getRegionInstance().destruct();
			...
			case 0:
				if(killer != null) {
					...
					if (BOSSES.contains(npc.getId())) {
						if(killer.getRights() == PlayerRights.DIAMOND_MEMBER || killer.getRights() == PlayerRights.RUBY_MEMBER) {
					...
					if(npc instanceof Zulrah)
						ZulrahInstance.startInstance(killer, true);
					/*
					 * Trio bosses
					 */
	...
	public void stop() {
		...
		if(npc.getId() == 1158 || npc.getId() == 1160) {
			...
			if(npc.getId() == 7005 || npc.getId() == 7010) {
				WiseOldMan.death(npc.getId(), npc.getPosition());
