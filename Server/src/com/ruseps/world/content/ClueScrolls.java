...
public class ClueScrolls {
	...
	private static final int[] LOW_LEVEL_REWARD = { 1077, 1089, 1107, 1125, 1131, 1129, 1133, 1511, 1168, 1165, 1179,
			1195, 1217, 1283, 1297, 1313, 1327, 1341, 1361, 1367, 1426, 2633, 2635, 2637, 7388, 7386, 7392, 7390, 7396,
			7394, 2631, 7364, 7362, 7368, 7366, 2583, 2585, 2587, 2589, 2591, 2593, 2595, 2597, 7332, 7338, 7350,
			7356 };

	private static final int[] MEDIUM_LEVEL_REWARD = { 2599, 2601, 19323, 19323, 19325, 19325, 19327, 19327, 19329, 19329, 19331, 19331, 2603, 2605, 2607, 2609, 2611, 2613, 7334, 7340, 7346,
	...
	private static final int[] HIGH_LEVEL_REWARD = { 1079, 1093, 1113, 1127, 1147, 1163, 1185, 1201, 1275, 1303, 1319,
			...
			2671, 2673, 2675, 7342, 7348, 7454, 7460, 7374, 7376, 7382, 7384, 7398, 7399, 7400, 3481, 3483, 3485, 3486,
	...
	private static final int[] BASIC_STACKS = { 995, 380, 561, 886, 374, 563, 890, 386, 560, 892,15273 };

	private static final String[] HINTS = { "Dig somewhere in Edgeville bank",
			...
			"Dig near the fourth minigame teleport", "Dig somewhere near ::cw", "Dig somewhere at Varrock sq", "Dig somewhere near some chickens", "Dig somewhere on the middle of barrows", "Dig in the cave of the giant bird", "Dig near the first few crawling hands"};

	public static void addClueReward(Player player) {
		if (player.getInventory().getFreeSlots() < 4) {
			player.getPacketSender().sendMessage("You must have atleast 4 free inventory spaces!");
			return;
		}
		player.getInventory().delete(2714, 1);
		player.getInventory().add(LOW_LEVEL_REWARD[Misc.getRandom(LOW_LEVEL_REWARD.length - 1)], 1);
		player.getInventory().add(LOW_LEVEL_REWARD[Misc.getRandom(LOW_LEVEL_REWARD.length - 1)], 1);
		player.getInventory().add(BASIC_STACKS[Misc.getRandom(BASIC_STACKS.length - 1)], RandomUtility.RANDOM.nextInt(200) + Misc.random(60));

		if (RandomUtility.RANDOM.nextInt(3) == 2) {
			player.getInventory().add(MEDIUM_LEVEL_REWARD[Misc.getRandom(MEDIUM_LEVEL_REWARD.length - 1)], 1);
		}
		if (RandomUtility.RANDOM.nextInt(10) == 5) {
			player.getInventory().add(HIGH_LEVEL_REWARD[Misc.getRandom(HIGH_LEVEL_REWARD.length - 1)], 1);
		}
