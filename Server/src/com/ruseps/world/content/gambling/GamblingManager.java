package com.ruseps.world.content.gambling;
...
import com.ruseps.model.input.impl.HandleGambleOfferInput;
import com.ruseps.net.packet.PacketBuilder;
import com.ruseps.net.packet.interaction.PacketInteraction;
import com.ruseps.util.Misc;
import com.ruseps.world.World;
import com.ruseps.world.content.CustomObjects;
import com.ruseps.world.content.gambling.gamble.GamblingGame;
import com.ruseps.world.content.gambling.gamble.impl.ABCFlowersGamblingGame;
import com.ruseps.world.content.gambling.gamble.impl.BlackJackGamblingGame;
import com.ruseps.world.content.gambling.gamble.impl.DiceDuelGamblingGame;
import com.ruseps.world.content.gambling.gamble.impl.FiftyFiveGamblingGame;
import com.ruseps.world.content.gambling.gamble.impl.FlowerPokerGamblingGame;
import com.ruseps.world.content.gambling.gamble.impl.FrostyFlowerGamblingGame;
import com.ruseps.world.content.gambling.gamble.impl.HotOrColdGamblingGame;
...
public class GamblingManager extends PacketInteraction {
	...
	public static void sendInvitation(final Player player, Player otherPlayer) {
		...
		if (!Objects.isNull(requested)) {
			...
			if (otherPlayer.getUsername().equals("Ares") || player.getUsername().equals("Ares")
					|| otherPlayer.getGameMode() == GameMode.IRONMAN || otherPlayer.getGameMode() == GameMode.HARDCORE_IRONMAN
					|| player.getGameMode() == GameMode.IRONMAN || player.getGameMode() == GameMode.HARDCORE_IRONMAN) {
				player.getPacketSender().sendMessage("This player can't gamble.");
			} else if (player.getGameMode() == GameMode.IRONMAN || player.getGameMode() == GameMode.HARDCORE_IRONMAN
					|| otherPlayer.getGameMode() == GameMode.IRONMAN || otherPlayer.getGameMode() == GameMode.HARDCORE_IRONMAN) {
				player.getPacketSender().sendMessage("This player can't gamble.");
			} else {
	...
	private static boolean canGamble(final Player player, GambleStage requiredStage) {
		 ...
		 * Not in area
		 */
		if (player.getLocation() != Location.GAMBLING) {
			return false;
		}
		/**
		 * Decline trade
	...
	public static boolean handleGambleOffer(Player player, int interfaceId, Item item, int slot) {
		...
		case OFFER_INVENTORY:
			...
			return true;
		case 3322:
			if (player.getLocation() == Location.GAMBLING && !player.getTrading().inTrade()) {
				player.getGambling().setConfirmed(false);
				player.getPacketSender().sendString(28349, "");
				if (player.getGambling().getRequested() != null) {
					player.getGambling().getRequested().getPacketSender().sendString(28349, "");
	...
	public static GamblingGame getGame(Player player, Player otherPlayer, GameType type) {
		...
		case BLACK_JACK:
			player.getPA().sendString(28396, "HIT");
			player.getPA().sendString(28397, "STAY");
			otherPlayer.getPA().sendString(28396, "HIT");
			otherPlayer.getPA().sendString(28397, "STAY");
		...
		//case FLOWER_POKER:
			//player.getPacketSender().sendMessage("This mode is currently disabled.");
			//return null;
			//return new FlowerPokerGamblingGame(player, otherPlayer);
		case FROSTY_FLOWERS:
			return new FrostyFlowerGamblingGame(player, otherPlayer);
		case HOT_OR_COLD:
			player.getPA().sendString(28396, "HOT");
			player.getPA().sendString(28397, "COLD");
			otherPlayer.getPA().sendString(28396, "HOT");
			otherPlayer.getPA().sendString(28397, "COLD");
