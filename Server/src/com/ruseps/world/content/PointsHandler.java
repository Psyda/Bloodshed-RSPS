...
public class PointsHandler {
	...
	public PointsHandler refreshPanel() {
		p.getPacketSender().sendString(26719, "");
		p.getPacketSender().sendString(26720, "@or2@  <img=40>@whi@ Player Statistics");

		p.getPacketSender().sendString(26723, "@or2@Trivia Points: @gre@"+triviaPoints);

		p.getPacketSender().sendString(26722, "@or2@Prestige Points: @gre@"+prestigePoints);
		p.getPacketSender().sendString(26726, "@or2@Commendations: @gre@ "+commendations);
		p.getPacketSender().sendString(26721, "@or2@Loyalty Points: @gre@"+(int)loyaltyPoints);
		p.getPacketSender().sendString(26728, "@or2@Bossing Points: @gre@ "+p.getBossPoints());

		p.getPacketSender().sendString(26727, "@or2@Dung. Tokens: @gre@ "+dungTokens);
		p.getPacketSender().sendString(26724, "@or2@Voting Points: @gre@ "+votingPoints);
		p.getPacketSender().sendString(26729, "@or2@Slayer Points: @gre@"+slayerPoints);
		p.getPacketSender().sendString(26730, "@or2@Pk Points: @gre@"+pkPoints);
		p.getPacketSender().sendString(26725, "@or2@Donation Points: @gre@"+donationPoints);

		p.getPacketSender().sendString(26731, "@or2@Wilderness Killstreak: @gre@"+p.getPlayerKillingAttributes().getPlayerKillStreak());
		p.getPacketSender().sendString(26732, "@or2@Wilderness Kills: @gre@"+p.getPlayerKillingAttributes().getPlayerKills());		
		p.getPacketSender().sendString(26733, "@or2@Wilderness Deaths: @gre@"+p.getPlayerKillingAttributes().getPlayerDeaths());
		p.getPacketSender().sendString(26734, "@or2@Arena Victories: @gre@"+p.getDueling().arenaStats[0]);
		p.getPacketSender().sendString(26735, "@or2@Arena Losses: @gre@"+p.getDueling().arenaStats[1]);
