...
public class Lottery {
	 ...
	 */
	private static String LAST_WINNER = "";
	...
	public static void restartLottery() {
		...
		try {
			...
			if(winner != null) {
				...
				writer.write("");
				writer.close();
				World.sendMessage("<col=D9D919><shad=0>[Lottery]</shad> @bla@This week's lottery winner is "+winner+"! Congratulations!");
			} else
	...
	public static void rewardPlayer(Player player, boolean ignore) throws IOException {
		if((!LAST_WINNER_REWARDED || ignore) && LAST_WINNER.equalsIgnoreCase(player.getUsername())) {
			...
			player.getPacketSender().sendMessage("@red@[Lottery] You've won the lottery for this week! Congratulations!");
			player.getPacketSender().sendMessage("@red@[Lottery] The reward has been added to your money pouch.");
			BufferedWriter writer = new BufferedWriter(new FileWriter(LAST_WINNER_FILE_LOCATION));
