...
/**
 * @author: @Austin Redid Messy Shit
 * Warning:
 * This crap is so messy and ugly. Will redo it once I get some time over.
 * Should be dupe-free.**austin added dung dupe fix
 */
...
public class Trading {
	...
	public void requestTrade(Player player2) {
		...
		/**if (player.getRights().equals(PlayerRights.ADMINISTRATOR)) {
			player.getPacketSender().sendMessage("You can't trade as an administrator.");
			return;
		}**/
	...
	public void acceptTrade(int stage) {
		...
		int player2FreeSpaces = player2.getInventory().getFreeSlots();
		int player2SpacesAfterOffer = player2.getInventory().getFreeSlots() + player2.getTrading().offeredItems.size();
		int playerFreeSpaces = player.getInventory().getFreeSlots();
		int playerSpacesAfterOffer = player.getInventory().getFreeSlots() + player.getTrading().offeredItems.size();
		if(player2FreeSpaces < offeredItems.size() && player2SpacesAfterOffer < offeredItems.size()) {
			player.getPacketSender().sendMessage("@red@[Trade Warning] @bla@" + player2.getUsername() + " only has " + player2SpacesAfterOffer + " free spaces.");
			player2.getPacketSender().sendMessage("@red@[Trade Warning]@bla@ " + "You must remove " + (offeredItems.size() - player2SpacesAfterOffer) + " items from your inventory to complete the trade.");
			return;
		}
		if(playerFreeSpaces < player2.getTrading().offeredItems.size() && playerSpacesAfterOffer < player2.getTrading().offeredItems.size()) {
			player2.getPacketSender().sendMessage("@red@[Trade Warning] @bla@" + player.getUsername() + " cannot accept with only " + playerSpacesAfterOffer + " free inventory spaces.");
			player.getPacketSender().sendMessage("@red@[Trade Warning]@bla@" + "You must remove " + (player2.getTrading().offeredItems.size() - playerSpacesAfterOffer) + " items from your inventory to complete the trade.");
			return;
		}
		if(stage == 2) {
	...
	public void giveItems() {
		...
		try {
			...
			for (Item item : player.getTrading().offeredItems) {
				PlayerLogs.log(player.getUsername(), "Gave item in trade to "+player2.getUsername()+". ItemName: "+item.getDefinition().getName()+", itemId: "+item.getId()+" amount: "+item.getAmount());
			}
			for (Item item : player2.getTrading().offeredItems) {
				PlayerLogs.log(player.getUsername(), "Receiving item from "+player2.getUsername()+". ItemName: "+item.getDefinition().getName()+", itemId: "+item.getId()+" amount: "+item.getAmount());
			}
