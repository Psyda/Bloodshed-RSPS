...
public class Kraken {
	...
	public void enter(Player player, boolean teleport) {
		...
		player.getKraken().setKrakenStage(KrakenStage.TELEPORTED_IN);
			...
			public void execute() {
				...
				if(tick == spawnTick) {
					whirlpool[0] = new NPC(149, new Position(3695, 5811, player.getPosition().getZ())).setSpawnedFor(player); //Boss
					World.register(whirlpool[0]);
					whirlpool[1] = new NPC(150, new Position(3700, 5809, player.getPosition().getZ())).setSpawnedFor(player);
					World.register(whirlpool[1]);
					whirlpool[2] = new NPC(150, new Position(3700, 5814, player.getPosition().getZ())).setSpawnedFor(player);
					World.register(whirlpool[2]);
					whirlpool[3] = new NPC(150, new Position(3692, 5809, player.getPosition().getZ())).setSpawnedFor(player);
					World.register(whirlpool[3]);
					whirlpool[4] = new NPC(150, new Position(3692, 5814, player.getPosition().getZ())).setSpawnedFor(player);
	...
	public void transformPool(Player player, NPC npc) {
		...
		tentacle[transformationId - 1] = new NPC(148, npc.getPosition()).setSpawnedFor(player);
	...
	public void spawnKraken(Player player, NPC npc) {
		kraken = new NPC(3847, npc.getPosition()).setSpawnedFor(player);
