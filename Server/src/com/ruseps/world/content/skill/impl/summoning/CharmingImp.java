...
public class CharmingImp {
	...
	private static void turnIntoXp(Player player, int itemId, int amount) {
		switch (itemId) {
		case GOLD_CHARM:
			if(player.getEquipment().contains(14932)) {
				player.getSkillManager().addExperience(Skill.SUMMONING, 1544 * amount * 2);
	        } else {
	        	player.getSkillManager().addExperience(Skill.SUMMONING, 1544 * amount);
	        }
			break;
		case GREEN_CHARM:
			if(player.getEquipment().contains(14932)) {
				player.getSkillManager().addExperience(Skill.SUMMONING, 2444 * amount * 2);
	        } else {
	        	player.getSkillManager().addExperience(Skill.SUMMONING, 2444 * amount);
	        }
			break;
		case CRIM_CHARM:
			if(player.getEquipment().contains(14932)) {
				player.getSkillManager().addExperience(Skill.SUMMONING, 3877 * amount * 2);
	        } else {
	        	player.getSkillManager().addExperience(Skill.SUMMONING, 3877 * amount);
	        }
			break;
		case BLUE_CHARM:
			if(player.getEquipment().contains(14932)) {
				player.getSkillManager().addExperience(Skill.SUMMONING, 5544 * amount * 2);
	        } else {
	        	player.getSkillManager().addExperience(Skill.SUMMONING, 5544 * amount);
	        }
			break;
		}
