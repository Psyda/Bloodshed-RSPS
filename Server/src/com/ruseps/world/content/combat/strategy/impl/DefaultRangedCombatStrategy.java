...
public class DefaultRangedCombatStrategy implements CombatStrategy {
	...
	public boolean canAttack(Character entity, Character victim) {
		...
		if (victim.isPlayer()) {//this is whts stopping from hitting in wildy btw if u didnt know.
			if(player.getRangedWeaponData() == RangedWeaponData.MAGMA_BLOWPIPE) {
				player.getPacketSender().sendMessage("You cannot use this weapon on another player");
			}
			if(player.getEquipment().contains(16337) || player.getEquipment().contains(16887)
					|| player.getEquipment().contains(20998)) {
				player.getPacketSender().sendMessage("This weapon is forbidden in the wilderness!");
	 ...
	 */
private boolean checkAmmo(Player player) {//can try this someone fucked ur blowpipe all up all u lol
	...
	public static void decrementAmmo(Player player, Position pos) {
		...
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 12927) {
			if (!player.getMagmaBlowpipeLoading().getContents().isEmpty()) {
				for (Entry<Integer> dart : player.getMagmaBlowpipeLoading().getContents().entrySet()) {
					player.getMagmaBlowpipeLoading().getContents().remove(dart.getElement());
					return;
				}
			} else {
