...
public enum CombatSpecial {
	...
	ABYSSAL_WHIP(new int[] { 4151, 15441, 15442, 15443, 15444 }, 50, 1, 1, CombatType.MELEE,
	...
	VINE_WHIP(new int[] { 20061, 4705 }, 75, 1.6, 1.6, CombatType.MELEE, WeaponInterface.WHIP) {
		@Override
		public CombatContainer container(Player player, Character target) {
			...
			if (target.isPlayer()) {
				...
				p.getSkillManager().setCurrentLevel(Skill.PRAYER,
						player.getSkillManager().getCurrentLevel(Skill.PRAYER) - amountToDrain);
				p.getPacketSender().sendMessage(
						"Your prayer has been drained " + amountToDrain + " prayer points from the vine whip spec!");

			}
			player.getPacketSender().sendMessage(
					"You drain your opponents prayer by a random amount of " + amountToDrain + " prayer points");
	...
	TOXIC_BLOWPIPE(new int[] { 12926, 12927 }, 50, 1.35, 2.20, CombatType.RANGED, WeaponInterface.DART) {
		@Override
		public CombatContainer container(Player player, Character target) {
			...
			TaskManager.submit(new Task(1, player, false) {
				...
				public void execute() {
					...
					new CombatHitTask(player.getCombatBuilder(),
							new CombatContainer(player, target, 2, CombatType.RANGED, true)).handleAttack();
		...
		public CombatContainer container(Player player, Character target) {
			...
			if (target instanceof Player) {
				...
				if (CurseHandler.isActivated(victim, CurseHandler.DEFLECT_MELEE)
						|| CurseHandler.isActivated(victim, CurseHandler.DEFLECT_MAGIC)
						|| CurseHandler.isActivated(victim, CurseHandler.DEFLECT_MISSILES)
						|| CurseHandler.isActivated(victim, CurseHandler.DEFLECT_SUMMONING)) {
				...
				if (PrayerHandler.isActivated(victim, PrayerHandler.PROTECT_FROM_MAGIC)
						|| PrayerHandler.isActivated(victim, PrayerHandler.PROTECT_FROM_MELEE)
						|| PrayerHandler.isActivated(victim, PrayerHandler.PROTECT_FROM_MISSILES)) {
	...
	LENT_WHIP(new int[] { 13444 }, 1, 6.55, 8.9, CombatType.MELEE, WeaponInterface.TWO_HANDED_SWORD) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(11989));
			player.performGraphic(new Graphic(2113));
