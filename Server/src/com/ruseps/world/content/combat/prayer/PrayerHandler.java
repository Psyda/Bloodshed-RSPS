	...
	private static void startDrain(final Player player) {
		...
		TaskManager.submit(new Task(1, player, true) {
			...
			public void execute() {
				...
				//	return;
				if((player.getRights() == PlayerRights.RUBY_MEMBER || player.getRights() == PlayerRights.MODERATOR || player.getRights() == PlayerRights.ADMINISTRATOR || player.getRights() == PlayerRights.MANAGER || player.getRights() == PlayerRights.DEVELOPER) && player.getLocation() != Location.WILDERNESS) {
					return;
				}
				if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
					for (int i = 0; i < player.getPrayerActive().length; i++) {
