		...
		public void handleAttack() {
			...
			if(victim.getCombatBuilder().getAttackTimer() <= 2) {
				if (victim.isPlayer()) {
					...
					if(!(((NPC)victim).getId() >= 6142 && ((NPC)victim).getId() <= 6145) && !(victim instanceof Zulrah))
			...
			if(attacker.isNpc() && victim.isPlayer()) {
				...
				if (victim.isNpc()) {
					switch(victim.asNpc().getId()){
						case 13447: Nex.takeDamage(player, damage);break;
						case GanodermicConstants.GANDOMERIC_BEAST_MOB_ID: GanodermicBeastCombat.takeDamage(player, damage);break;
					}
				} else {
