...
public final class CombatFactory {
	...
	public static final double PRAYER_ACCURACY_REDUCTION = .355;
	...
	public static boolean combatBracelet(Character entity) {
		return ((Player) entity).getEquipment().contains(11120);
	}

	public static boolean fullTrickster(Character entity) {
		return ((Player) entity).getEquipment().containsAll(20020, 20019, 20012, 20011, 20010);
	}

	public static boolean twistedBow(Character entity) {
		return ((Player) entity).getEquipment().contains(20998);
	}
	...
	public static Hit getHit(Character entity, Character victim, CombatType type) {
		switch (type) {
		case MELEE:
			return new Hit(RandomUtility.inclusiveRandom(1,
		...
		case MAGIC:
			return new Hit(RandomUtility.inclusiveRandom(1, DesolaceFormulas.getMagicMaxhit(entity)), Hitmask.RED,
					CombatIcon.MAGIC);
		case DRAGON_FIRE:
	...
	public static boolean rollAccuracy(Character attacker, Character victim, CombatType type) {
		...
		if (attacker.isPlayer() && victim.isPlayer()) {
			...
			case MAGIC:
				...
				System.out.print(Misc.getRandom(DesolaceFormulas.getMagicDefence(p2)) < Misc.getRandom((mageAttk / 2))
	...
	public static int calculateMaxRangedHit(Character entity, Character victim) {
		...
		if (CombatFactory.combatBracelet(player)){
		...
		if (CombatFactory.twistedBow(player)){
		...
		if (CombatFactory.fullTrickster(player)){
	...
	public static boolean checkHook(Character entity, Character victim) {
		...
		if (victim.isNpc() && entity.isPlayer()) {
			...
			if(npc.getLocation().equals(Location.ZULRAH) && npc.getId() == 2042){
				if(((Zulrah) npc).getState().equals(ZulrahState.ANIMATING)){
					entity.asPlayer().getCombatBuilder().reset(true);
					return false;
				}
			}
			...
			if (Nex.nexMob(npc.getId())) {
				...
				if(entity.getCombatBuilder().getStrategy().getCombatType() == CombatType.MELEE) {
					((Player)entity).getPacketSender().sendMessage("You cannot reach Zulrah with melee!");
					entity.getCombatBuilder().reset(true);
					return false;
				}
			}
	...
	protected static void applyPrayerProtection(CombatContainer container, CombatBuilder builder) {
		...
		if (builder.getVictim().isPlayer()) {
			...
			if (builder.getCharacter().isNpc()) {
				...
				if (PrayerHandler.isActivated(victim, PrayerHandler.getProtectingPrayer(container.getCombatType()))
					...
					container.allHits(context -> {
						...
						if (attacker.getId() == 2745) { // Jad
							...
							double reduceRatio = attacker.getId() == 1158 || attacker.getId() == 7005 || attacker.getId() == 7010 || attacker.getId() == 1160 ? 0.4 : 0.8;
				...
				if (PrayerHandler.isActivated(victim, PrayerHandler.getProtectingPrayer(container.getCombatType()))
					...
					container.allHits(context -> {
						...
						/*
						 * mod = Math.round(RandomUtility.RANDOM.nextDouble() *
						 * 100.0) / 110.0; if (mod <=
						 * CombatFactory.PRAYER_ACCURACY_REDUCTION) {
						 * context.setAccurate(false); }
						 */
					});
				}
			...
			if (npc.getId() == 8349 && container.getCombatType() == CombatType.MELEE) {
			...
			} else if(npc.getId() == 7005 && container.getCombatType() == CombatType.MELEE) {
				container.allHits(context -> {
					int hit = context.getHit().getDamage();
					double mod = Math.abs(1 - 0.5);
					context.getHit().incrementAbsorbedDamage((int)(hit - (hit * mod)));
				...
				if(container.getCombatType() == CombatType.MELEE) {
					attacker.getPacketSender().sendMessage("Your attack has"+(!container.getHits()[0].isAccurate() ? "" : " close to")+" no effect on the wise old man.");
				}
			} else if(npc.getId() == 7010 && (container.getCombatType() == CombatType.MAGIC || container.getCombatType() == CombatType.RANGED)) {
				container.allHits(context -> {
					int hit = context.getHit().getDamage();
					double mod = Math.abs(1 - 0.95);
					context.getHit().incrementAbsorbedDamage((int)(hit - (hit * mod)));
				...
				attacker.getPacketSender().sendMessage("Your "+(container.getCombatType() == CombatType.MAGIC ? "magic" : "range")+" attack has"+(!container.getHits()[0].isAccurate() ? "" : " close to")+" no effect on the angry old man.");
