...
public class NPCDropTableChecker {
	...
	public static final List<Integer> searchedNpcIds = new ArrayList<Integer>();
	...
	public boolean handleButtonClick(final Player player, final int actionId) {
		if (actionId >= -27885 && actionId <= -27716) {
			...
			if (player.searchedNpc()) {
				if (index >= ItemSearch.npcIds.size() || ItemSearch.npcIds.get(index) == null) {
					return true;
				}
				
				int childId = 37651;
				for (String npcName : ItemSearch.dropTableNpcNames) {
					player.getPacketSender().sendFrame126(npcName, childId++);
				}
				
				showNPCDropTable(player, ItemSearch.npcIds.get(index));
				player.getPacketSender().sendFrame126("@or1@You are now viewing the @whi@"
						+ ItemSearch.dropTableNpcNames.get(index) + "@or1@ drop table.", 37605);
				player.getPacketSender().sendFrame126("@or1@You are now viewing the @whi@"
						+ dropTableNpcNames.get(index) + "@or1@ drop table.", 37605);
				player.npcDropTableDelay = System.currentTimeMillis();
				player.setsearchedNpc(false);
				
				ItemSearch.npcIds.clear();
				ItemSearch.dropTableNpcNames.clear();
	 ...
	 */
	public static void showNPCDropTable(final Player player, final int npcId) {
