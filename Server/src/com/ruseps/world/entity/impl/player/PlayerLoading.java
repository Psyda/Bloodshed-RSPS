...
public class PlayerLoading {
	...
	public static int getResult(Player player, boolean force) {
		...
		try (FileReader fileReader = new FileReader(file)) {
			...
			if (reader.has("magmablowpipe-charge-item") || reader.has("magmablowpipe-charge-amount")) {
				player.getMagmaBlowpipeLoading().getContents().setCount(reader.get("magmablowpipe-charge-item").getAsInt(),
						reader.get("magmablowpipe-charge-amount").getAsInt());
			}
			...
			if (reader.has("set-home")) {
				player.setHome(builder.fromJson(reader.get("set-home"), Position.class));
			}
