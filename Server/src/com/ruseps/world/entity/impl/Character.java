...
public abstract class Character extends Entity {
	...
	public void lock(int time) {
		...
		if(isPlayer()){
			final int delay = (int) ((TimeUnit.MILLISECONDS.convert(time, TimeUnit.SECONDS) / 600));
			TaskManager.submit(new Task(delay, false) {
				@Override
				protected void execute() {
					getMovementQueue().setLockMovement(false);
					asPlayer().sendMessage("Your stun has been lifted!");
					stop();
				}
			});
		}
