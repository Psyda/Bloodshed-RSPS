...
public class PlayerUpdating {
	...
	public static void update(final Player player) {
		...
		for (Iterator<Player> playerIterator = player.getLocalPlayers().iterator(); playerIterator.hasNext();) {
			...
			if (World.getPlayers().get(otherPlayer.getIndex()) != null && otherPlayer.isVisible() && !otherPlayer.isHidePlayer() && otherPlayer.getPosition().isWithinDistance(player.getPosition()) && !otherPlayer.isNeedsPlacement()) {
		...
		for(Player otherPlayer : World.getPlayers()) {
			if (player.getLocalPlayers().size() >= 79 || playersAdded > MAX_NEW_PLAYERS_PER_CYCLE)
				break;
			if (otherPlayer == null || otherPlayer == player || !player.isVisible() || otherPlayer.isHidePlayer() || player.getLocalPlayers().contains(otherPlayer) || !otherPlayer.getPosition().isWithinDistance(player.getPosition()))
				continue;
