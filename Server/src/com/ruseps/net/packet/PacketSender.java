...
public class PacketSender {
    ...
    public PacketSender sendInterfaceRemoval() {
        ...
        if(player.getDicing().inDice) {
			sendClientRightClickRemoval();
			player.getDicing().declineDice(true);
		}
    ...
    public PacketSender sendProjectile(Projectile projectile){

        if(projectile.getEntityLockID() == 0)
            return sendFinalizedProjectile(projectile.getStartPosition(), projectile.getDestination(), projectile.getInitialSlope(), projectile.getDuration(), projectile.getPropelledGraphicID(), projectile.getInitialHeight(), projectile.getFinalHeight(), projectile.getDelay(), projectile.getCoveredTilesBySource(), projectile.getStartPositionOffset());

        sendPosition(projectile.getStartPosition());

        player.getSession().queueMessage(projectile.construct());
        return this;
    }

    public PacketSender sendFinalizedProjectile(Position position, Position end, int initialSlope, int duration, int movingGraphicID, int startHeight, int endHeight, int startDelay, int creatorSize, int startDistanceOffset){
        final Position other = player.getLastKnownRegion();
        sendPosition(position);
        PacketBuilder out = new PacketBuilder(118);
        out.put(end.getX() - 8 * other.getRegionX(), ValueType.C);
        out.put(end.getY() - 8 * other.getRegionY(), ValueType.C);
        out.putShort(movingGraphicID);
        out.put(startHeight);
        out.put(endHeight);
        out.putShort(startDelay);
        out.putShort(duration);
        out.put(initialSlope);
        out.put((creatorSize * 64) + (startDistanceOffset * 64));
        player.getSession().queueMessage(out);
        return this;
    }
    public PacketSender sendScreenFade(String text) {
        PacketBuilder out = new PacketBuilder(9, Packet.PacketType.BYTE);
        out.putString(text);
        player.getSession().queueMessage(out);
        return this;
    }
