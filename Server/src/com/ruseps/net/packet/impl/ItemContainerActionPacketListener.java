...
import com.ruseps.model.input.impl.EnterAmountToGamble;
...
import com.ruseps.model.input.impl.EnterAmountToRemoveFromGamble;
	...
	private static void firstAction(Player player, Packet packet) {
		...
		case -28482:
		  ...
		  case Trading.INTERFACE_ID:
			   ...
			   } else if(player.getGamble().inGamble()) {
			    player.getGamble().gambleItem(id, 1, slot);
			   }
			   break;
		  case Trading.INTERFACE_REMOVAL_ID:
			   if(player.getTrading().inTrade())
			    ...
			    player.getGamble().removeGambledItem(id, 1);
			   break;
		...
		case -28482:
		  ...
		  case Trading.INTERFACE_ID:
			   if(player.getTrading().inTrade()) {
			    ...
			    player.getGamble().gambleItem(id, 5, slot);
			   }
			   break;
		  case Trading.INTERFACE_REMOVAL_ID:
			   if(player.getTrading().inTrade())
			    ...
			    player.getGamble().removeGambledItem(id, 5);
			   break;
		case Dueling.INTERFACE_REMOVAL_ID:
			if (Dueling.checkDuel(player, 1) || Dueling.checkDuel(player, 2)) {
		...
		case Equipment.INVENTORY_INTERFACE_ID:
		  ...
		  case Trading.INTERFACE_ID:
			   if(player.getTrading().inTrade()) {
			    ...
			    player.getGamble().gambleItem(id, 10, slot);
			   }
			   break;
		  case Trading.INTERFACE_REMOVAL_ID:
			   if(player.getTrading().inTrade())
			    ...
			    player.getGamble().removeGambledItem(id, 10);
			   break;
		case Dueling.INTERFACE_REMOVAL_ID:
			if (Dueling.checkDuel(player, 1) || Dueling.checkDuel(player, 2)) {
		...
		case -31915:
		  ...
		  case Trading.INTERFACE_ID:
			   if(player.getTrading().inTrade()) {
			    ...
			    player.getGamble().gambleItem(id, player.getInventory().getAmount(id), slot);
			   }
			   break;
		  case Trading.INTERFACE_REMOVAL_ID:
			   if(player.getTrading().inTrade()) {
			    ...
			    for(Item item: player.getGamble().offeredItems) {
			     if(item != null && item.getId() == id) {
			      player.getGamble().removeGambledItem(id, item.getAmount());
			      if(ItemDefinition.forId(id) != null && ItemDefinition.forId(id).isStackable())
			       break;
			     	}
			      }
			...
			if (player.isBanking()) {
				if (interfaceId == 11) {
					...
					if(player.getBank(Bank.getTabForItem(player, id)).getAmount(id) == 1) {
						player.getPacketSender().sendMessage("You only have 1 " + ItemDefinition.forId(id).getName() + "!");
					} else {
						player.getBank(player.getCurrentBankTab()).switchItem(player.getInventory(),
								new Item(id, player.getBank(Bank.getTabForItem(player, id)).getAmount(id) - 1), slot, true, true);
					}
					player.getBank(player.getCurrentBankTab()).open(player, false);
				}
			}
