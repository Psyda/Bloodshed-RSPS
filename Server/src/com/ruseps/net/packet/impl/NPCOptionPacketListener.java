			...
			public void execute() {
				...
				switch(npc.getId()) {
					...
					case 367:
						...
						//DialogueManager.sendStatement(player, "Temporarily disabled.");
					...
					break;
					case 607:
						TeleportHandler.teleportPlayer(player, new Position(3367, 3936), TeleportType.NORMAL);
						player.getPacketSender().sendMessage("@blu@You have teleported to lava bidge");
						player.getPacketSender().sendMessage("@gre@Have 3 shards in your inventory to create malediction or odium ward.");
						break;
					case 611:
                        if(player.getInventory().containsAll(4660, 4666, 4670)) {
                            player.getInventory().delete(4660, 1).delete(4666, 1).delete(4670, 1).add(11926, 1);
                            player.getPacketSender().sendMessage("You forge the shards together to create an odium ward!");
                        }
                        if(player.getInventory().containsAll(4672, 4680, 4686)) {
                            player.getInventory().delete(4672, 1).delete(4680, 1).delete(4686, 1).add(11924, 1);
                            player.getPacketSender().sendMessage("You forge the shards together to create a malediction ward!");
                        }
                        break;
					case 465:
					...
					break;
						...
						ShopManager.getShops().get(5).open(player);
						break;
					case 2313://ANTI-LEECH
						//player.getPacketSender().sendEnterAmountPrompt("Enter Amount");
						//player.setInputHandling(new EnterAmountOfBonesOnAltar());
						break;
					case 710:
				...
				case 568:
					...
					player.setDialogueActionId(277);
					break;
				case 9713:
				...
				case 8725:
					...
					break;
				case 4249:
					DialogueManager.sendStatement(player, "Right click a player to start gambling/dicing.");
	...
	public void handleFourthClick(Player player, Packet packet) {
		...
		player.setWalkToTask(new WalkToTask(player, npc.getPosition(), npc.getSize(), new FinalizedMovementTask() {
			...
			public void execute() {
				...
				case 4657:
					if(player.getRights() == PlayerRights.PLAYER) {
						player.getPacketSender().sendMessage("You need to be a member to teleport to this zone.").sendMessage("To become a member, visit Elysian-PS.com and check the store for more information.");
						return;
					}
