...
import com.ruseps.world.content.ChaosTunnelHandler;
		...
		player.setInteractingObject(gameObject).setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
			...
			public void execute() {
				...
				if (player.getTeleblockTimer() >0) {
					player.getPacketSender().sendMessage("You cannot teleport while you are teleblocked.");
					return;
				}
				...
				case 7808:
					...
					break;
					//Dakings stuff to add - Levers
				case 5960: //teleports outside mage bank
				...
				case 9707:
					if (gameObject.getPosition().getX() == 3105 && gameObject.getPosition().getY() == 3952)
						TeleportHandler.teleportPlayer(player,  new Position(3105, 3956), TeleportType.LEVER);
										break;
				case 5959: //teleports inside mage bank
					player.setDirection(Direction.WEST);
					TeleportHandler.teleportPlayer(player, new Position(2539, 4712), TeleportType.LEVER);
					break;					
				case 2971: //shimmering wall
					if(gameObject.getPosition().getX() == 2420 && gameObject.getPosition().getY() == 4690) {
						player.moveTo(new Position(2420, 4691, 0));
					} else if(gameObject.getPosition().getX() == 2421 && gameObject.getPosition().getY() == 4690) {
						player.moveTo(new Position(2421, 4690, 0));
					}
					break;
					case 2878:// Sparkling pool
					if (gameObject.getPosition().getX() == 2541 && gameObject.getPosition().getY() == 4719) {
						player.moveTo(new Position(2508, 4694, 0));
					}
					break;
				case 2879://sparkling pool
					if (gameObject.getPosition().getX() == 2508 && gameObject.getPosition().getY() == 4686) {
				...
				case 26427:
					...
					if(!leaveRoom && player.getMinigameAttributes().getGodwarsDungeonAttributes().getKillcount()[index] < 20) {
						if (player.getAmountDonated() > 49 || player.getRights() == PlayerRights.SILVER_MEMBER || player.getRights() == PlayerRights.GOLD_MEMBER) {
				...
				case 9357:
					...
					break;
				case 9356:
					player.setDialogueActionId(1000);
		...
		player.setInteractingObject(gameObject).setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
			public void execute() {
				...
				case 312:
					...
					break;
				case 357:
					player.getPacketSender().sendMessage("Fuck you!");
                	//player.getPointsHandler().incrementDonationPoints(5);
					//player.incrementAmountDonated(25);
					//player.getPointsHandler().refreshPanel();
					//PlayerPanel.refreshPanel(player);
